...
public:
  ...
  virtual
  void
  filter (const PairHashTable::HashTable & minimaSet, const SC_PartitionFunction::Z_Matrix & Z,
	  const size_t curMinIndex, const MyState currentMin, IndexList & indicesToFilter) = 0;
...
public:
   ...
   *
   */
  NeighMinCombinator (NeighMinFilter & first, NeighMinFilter & second) :
...
public:
   ...
   */
  GreaterByZMatrix (const SC_PartitionFunction::Z_Matrix &Zmatrix, const size_t row) :
