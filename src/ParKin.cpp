...
//! the maximal number of neigbored rna-structures.
int MaximalNeighbors = 0;
/**
 * ! reporter function for browse_neighs() from the ViennaRNA-Package.
 */
int
countNeighbor (struct_en* a, struct_en* b)
{
  MaximalNeighbors++;
 ...
 *  and return the maximum number of neighbored RNA structures that current State has
 *  @param rnaSeq, represents RNA sequence.
 */
size_t
getMaxNeighborNumber (std::string rnaSequence)
{
  MaximalNeighbors = 0;
  // create open chain State
  std::string openChain (rnaSequence.size (), '.');
  browse_neighs ((char*) rnaSequence.c_str (), (char*) openChain.c_str (), 0, 0, 0, countNeighbor);
 ...
 */
size_t
getMaximalNeighborsOfAMacroState (SC_PartitionFunction::Z_Matrix& z, std::set<size_t>& done_List,
				  std::unordered_map<size_t, MyState>& minimaForReverseSearch)
{

  size_t maxNeighbors = 0;
  size_t countNeighborsForState = 0;
  for (std::set<size_t>::iterator from = done_List.begin (); from != done_List.end (); from++)
    {
      ...
      if (z.find (basinID) != z.end ())
	{
	  ...
	  for (std::set<size_t>::iterator to = std::next (from, 1); to != done_List.end (); to++)
	    {
...
calculateRateMatrix (SC_PartitionFunction::Z_Matrix& z, std::set<size_t>& done_List,
		     std::unordered_map<size_t, MyState>& minimaForReverseSearch, size_t maxNeighNum)
{
  biu::MatrixSparseC<double>& final_Rate = *new biu::MatrixSparseC<double> (done_List.size (), done_List.size (),
									    0.0);
...
floodBasin (vrna_fold_compound_t *vc, flooderInputParameter* inParameter, flooderOutputParameter* outParameter)
{
  ...
  PairHashTable::HashTable& localThreadMinima = outParameter->LocalThreadMinima;
  ...
  localThreadMinima.insert (
    { *minState, loopCurrentLocalMinID });
  ...
  if (inParameter->Filter != NULL)
    {
      inParameter->Filter->filter (localThreadMinima, outParameter->PartitionFunctions, loopCurrentLocalMinID,
				   *minState, outParameter->IndicesOfFilteredMinima);
    }

  //add features for analysis.
  outParameter->Analysis.NumberOfStatesInBasin = outParameter->ScBasin->size ();
  outParameter->Analysis.NumberOfStatesOutOfBasin = spc.getNumberOfOuterStates ();
  outParameter->Analysis.TimeForStatesInBasin = 0;
  outParameter->Analysis.TimeForStatesOutOfBasin = 0;
  outParameter->Analysis.MaxEnergyThreshold = myFlooder.getFinalEnergyThreshold ();
 ...
 */
size_t
estimateMaxToHash (size_t numberOfHashMaps, size_t sequenceLength)
{
  //determine estimated maximum maxToHash value without swapping.
  size_t memInBytes = getTotalSystemMemoryInBytes ();
  size_t sizeOfHashEntry = sizeof(size_t) + sizeof(MyState) + sizeof(short) * (sequenceLength + 1);
...
// To parse the Program Arguments
  ...
  biu::COptionParser out_Parser = biu::COptionParser (allowed_Arg, argc, argv, info);
...
// check if there is any unparseable argument
  ...
  try
	...
	{
	  error_Mas << " start Structure " << rna_start_str << " is no valid structure";
	  throw ArgException (error_Mas.str ());
	}
      ...
      if (out_Parser.argExist ("finalStr"))
	{
	  ...
	  if (!StructureUtils::IsValidStructure (rna_final_str) || rna_final_str.size () == 0)
	    {
	      error_Mas << " final Structure " << rna_final_str << " is no valid structure";
	      throw ArgException (error_Mas.str ());
	    }
	}
      ...
      if (out_Parser.argExist ("partitionFunctions"))
	{
	  ...
	  if (out_Parser.getStrVal ("partitionFunctions").size () != 0)
	    {
	      partitionFunctionFileName = out_Parser.getStrVal ("partitionFunctions");
      ...
      if (out_Parser.argExist ("transProb"))
	{
	  ...
	  if (out_Parser.getStrVal ("transProb").size () == 0)
	    {
	      throw ArgException ("no transition probability output file given but argument present");
	    }
	  else if (out_Parser.getStrVal ("transProb").compare ("STDOUT") != 0)
	    {
	      if (out_Parser.getStrVal ("out").compare (out_Parser.getStrVal ("transProb")) == 0)
		{
		  transOut = out;
		}
	      else
		{
		  transOutFile = new std::ofstream (out_Parser.getStrVal ("transProb").c_str (),
						    std::ofstream::out);
		  if (!transOutFile->is_open ())
		    {
		      std::ostringstream oss;
		      oss << "cannot open output file '" << out_Parser.getStrVal ("transProb") << "'";
      ...
      if (out_Parser.argExist ("M"))
	{
	  ...
	  if (energyModelNumber >= 0 || energyModelNumber <= 2)
		...
		{
		case 0:
		  energyModelFile = path + shareFolder + "/rna_turner2004.par";
		  ...
		  break;
		case 1:
		  energyModelFile = path + shareFolder + "/rna_turner1999.par";
		  read_parameter_file (energyModelFile.c_str ());
		  break;
		case 2:
		  energyModelFile = path + shareFolder + "/rna_andronescu2007.par";
		  read_parameter_file (energyModelFile.c_str ());
		  break;
		}
	    }
	...
	{
	  neighborCombinator = new NeighMinCombinator (*neighbor_E_Filter, *neighbor_K_Filter);
      ...
      std::chrono::duration<double> elapsed_seconds_InitialWalk = endInitialWalk - startInitialWalk;
      std::cout << "Initial Walk elapsed time: " << elapsed_seconds_InitialWalk.count () << "s\n";

      // add current minimum to set of minima and store its index
      size_t currentMinID = 0;
      Minima.insert (
	{ *startStateMinimum, currentMinID });
      MinimaForReverseSearch.insert (
	{ currentMinID, *startStateMinimum });
	  ...
	  while (!toDo_List.empty ())
	    {
	      ...
	      int numThreads = std::min (listSize, maxThreads);

	      std::cout << "Todo list size: " << listSize << std::endl;

	      //create threads for local flooding:
	      std::pair<flooderInputParameter*, flooderOutputParameter*> threadParameter[listSize];

	      //init stopwatch for parallel loop:
	      std::chrono::time_point<std::chrono::system_clock> startParallelLoop, endParallelLoop;
	      startParallelLoop = std::chrono::system_clock::now ();

	      //set dynamic maxToHash
	      if (dynamicMaxToHash)
		maxToHash = estimateMaxToHash (numThreads, rnaSequence.length ());

	      //divide todolist in parallel parts.
#pragma omp parallel for num_threads(numThreads)
	      for (int k = 0; k < listSize; ++k)
		{
		  flooderInputParameter* inParameter = new flooderInputParameter ();
		  inParameter->BasinID = toDo_List[k];
		  inParameter->CurrentMinimum = &MinimaForReverseSearch.at (toDo_List[k]);
		  ...
		  flooderOutputParameter* outParameter = new flooderOutputParameter ();
		  ...
		  //print stopwatch for parallel loop:
	      endParallelLoop = std::chrono::system_clock::now ();
	      std::chrono::duration<double> elapsed_seconds_ParallelLoop = endParallelLoop - startParallelLoop;
	      std::cout << "Parallel loop elapsed time: " << elapsed_seconds_ParallelLoop.count () << "s\n";

	      //copy toDo_list to done (was done in parallel for)
	      for (std::vector<size_t>::iterator i = toDo_List.begin (); i != toDo_List.end (); i++)
		{
		  done_List.insert (*i);
		}
	      toDo_List.clear (); //it was done by the parallel for, and added to done before that.

	      //init stopwatch for merging of indices:
	      std::chrono::time_point<std::chrono::system_clock> startIndexMerging, endIndexMerging;
	      startIndexMerging = std::chrono::system_clock::now ();

	      //merge results.
	      for (int k = 0; k < listSize; ++k)
		{
		  std::pair<flooderInputParameter*, flooderOutputParameter*>* bothParameter;
		  bothParameter = &threadParameter[k];

		  flooderInputParameter* inParameter = bothParameter->first;
		  flooderOutputParameter* outParameter = bothParameter->second;

		  std::cout << "States out of basin: " << outParameter->Analysis.NumberOfStatesOutOfBasin
		      << std::endl;
		  std::cout << "States in basin: " << outParameter->Analysis.NumberOfStatesInBasin << std::endl;
		  std::cout << "Energy Threshold: " << outParameter->Analysis.MaxEnergyThreshold << std::endl;
		  std::cout << "Max Energy in Basin: " << outParameter->ScBasin->getMaxEnergy () << std::endl;
		  std::cout << "Processed States by flooding: " << outParameter->Analysis.ProcessedStates
		      << std::endl;
		  ...
		  if (writeDotplot)
		    {
		      ...
		      for (SC_DotPlot::DotPlot::iterator it = tmpDotPlot.begin (); it != tmpDotPlot.end (); ++it)
			{
		  ...
		  for (auto minIt = localThreadMinima.begin (); minIt != localThreadMinima.end (); minIt++)
		    {
			...
			{ //if local min is not in total list.
			  ...
			  Minima.insert (
			    { (localThreadMin), lowestMaxIndex });
			  MinimaForReverseSearch.insert (
			    { lowestMaxIndex, (localThreadMin) });
			}

		      size_t newIndex = Minima.at (localThreadMin);

		      //assingn total id to localID.
		      mapOldIndexToNewIndex.insert (
			{ oldIndex, newIndex });
		    }

		  SC_PartitionFunction::Z_Matrix& localZ = outParameter->PartitionFunctions;
		  //adjust index in partitionfunction z.
		  for (auto it = localZ.begin (); it != localZ.end (); it++)
		    {
		      if (mapOldIndexToNewIndex.find (it->first.first) != mapOldIndexToNewIndex.end ()
			  && mapOldIndexToNewIndex.find (it->first.second) != mapOldIndexToNewIndex.end ())
			{
			  size_t newFirstIndex = mapOldIndexToNewIndex.at (it->first.first);
			  size_t newSecondIndex = mapOldIndexToNewIndex.at (it->first.second);
			  SC_PartitionFunction::PairID localPairID (newFirstIndex, newSecondIndex);
			  z.insert (
			    { localPairID, it->second });
			}
		    }
		  ...
		  if (dynamicBestK)
			  ...
			  == dynamicBestKFilterNeighborList.end ())
			{
			  ...
			  size_t currentMinID = localThreadMinima.at (*inParameter->CurrentMinimum);
			  ...
			  // sort bz Z value
			  std::sort (allNeighboredMinima.begin (), allNeighboredMinima.end (), greaterByZ);

			  for (int nm = 0; nm < allNeighboredMinima.size (); ++nm)
			    {
			      allNeighboredMinima[nm] = mapOldIndexToNewIndex[allNeighboredMinima.at (nm)];
			    }

			  dynamicBestKFilterNeighborList.insert (
			    { loopCurrentMinID, allNeighboredMinima });
			}
		    }
		  ...
		  for (int i = 0; i < outParameter->IndicesOfFilteredMinima.size (); i++)
		    {
		      size_t index = mapOldIndexToNewIndex.at (outParameter->IndicesOfFilteredMinima.at (i));
			  ...
			  && std::find (toDo_List.begin (), toDo_List.end (), index) == toDo_List.end ())
			{
	      ...
	      //print stopwatch for index merging:
	      endIndexMerging = std::chrono::system_clock::now ();
	      std::chrono::duration<double> elapsed_seconds_IndexMerging = endIndexMerging - startIndexMerging;
	      std::cout << "Index merging elapsed time: " << elapsed_seconds_IndexMerging.count () << "s\n";

	      //search for the final structure (if given in parameters) and stop the exploration
	      // if the according basin is done.
	      if (rna_final_str.compare ("") != 0)
		{
		  ...
		  for (auto doneIt = done_List.begin (); doneIt != done_List.end (); ++doneIt)
		      ...
		      if (StructureUtils::IsEqual (MinimaForReverseSearch[*doneIt].structure, finalStructurePT))
			{
			  ...
			  finalStructureDone = true;
			  break;
			}
		    }
		  free (finalStructurePT);
		  finalStructurePT = NULL;
		  if (finalStructureDone)
		    {
		      break;
		    }
		}
	  ...
	  if (dynamicBestK)
	    {
	      if (!mfeFound)
		{
		  std::cout << "Warning: The mfe structure could not be found for K=" << filterValueK << std::endl;
		  ...
		  for (auto dynBKit = dynamicBestKFilterNeighborList.begin ();
		      dynBKit != dynamicBestKFilterNeighborList.end (); ++dynBKit)
		    {
		      if (dynBKit->second.size () >= filterValueK)
			{
			  size_t kNeighbor = dynBKit->second.at (filterValueK - 1);
			  if (done_List.find (kNeighbor) == done_List.end ()
			      && std::find (toDo_List.begin (), toDo_List.end (), kNeighbor) == toDo_List.end ())
      ...
      if (dynamicBestK)
	{
	  if (mfeFound)
	    {
	      std::cout << "DynamicBestK: The mfe structure could be found for K=" << filterValueK << std::endl;
	    }
	  else
	    {
	      std::cout << "Warning: The mfe structure could not be found by increasing K. Last K=" << filterValueK
		  << std::endl;
	    }
	}
      else
	{
	  if (!mfeFound)
	    {
	      std::cout << "Warning: The mfe structure could not be found!" << std::endl;
	    }
	}

      std::cout << std::endl;
      std::cout << "MFE structure: " << mfeStructure << std::endl;
      std::cout << "The start state is: " << startState.toString () << std::endl;
      std::cout << "The start state ends in basin: " << startStateMinimum->toString () << std::endl;
      std::cout << std::endl;

      // Calculate the final Rate Matrix:
      *transOut << "              --------------THE FINAL RATE MATRIX----------------------- " << std::endl;
      // the maximum number of macrostates is used to normalize the rates.
      size_t maxNeighNum = getMaximalNeighborsOfAMacroState (z, done_List, MinimaForReverseSearch);
      biu::MatrixSparseC<double>* final_Rate = &calculateRateMatrix (z, done_List, MinimaForReverseSearch,
								     maxNeighNum);
      ...
      for (size_t from = 0; from < done_List.size (); from++, fromIt++)
	{
	  ...
	  final_minima.insert (
	    { from, MinimaForReverseSearch.at (*fromIt) });
	}
      ...
      if (partitionFunctionFileName.size () > 0 & writePartitionFunctions)
	{
	  ...
	  ptFile << "              --------------THE PARTITIONFUNCTION MATRIX----------------------- " << std::endl
	      << std::endl;
	  ...
	  for (size_t i = 0; i < final_minima.size (); i++)
	    {
	      ...
	      partitionSumLandscape += z.at (PairID (minIndex, minIndex)).getZ ();
	    }
	  SC_DotPlot::DotPlot normalizedDotplot = SC_DotPlot::getBasePairProbabilities (dotplot,
											partitionSumLandscape);
	  bool dotplotWritten = SC_DotPlot::writeDotPlot_PS (dotPlotFileName, rnaSequence, normalizedDotplot);
	  if (!dotplotWritten)
	    {
	      std::cerr << "\nWarning: error during the writing of the dot plot file " << dotPlotFileName << "\n";
	    }
	  std::cout << "The overall partitionfunction is: " << partitionSumLandscape << std::endl;
	}
 ...
 */
void
writeDescription (biu::OptionMap & allowed, std::string & info)
{
...
//description
  ...
  allowed.push_back (
      biu::COption ("seq", false, biu::COption::STRING, "The RNA sequence of the molecule", "ACUGUAUGCGCGU"));
  allowed.push_back (
  ...
  allowed.push_back (
      biu::COption ("maxToHash", true, biu::COption::INT,
		    "Sets the maximum number of states to be hashed for a gradient walk.", std::to_string (SIZE_MAX)));
  allowed.push_back (
      biu::COption (
	  "dynamicMaxToHash",
	  true,
  ...
  allowed.push_back (
	  ...
	  "2"));
  allowed.push_back (
      biu::COption ("B", true, biu::COption::INT, "Set the temperature for the Boltzmann weight.", "37"));
  allowed.push_back (
  ...
  allowed.push_back (biu::COption ("energyFile", true, biu::COption::STRING, "File to store all energies."));
  allowed.push_back (
  ...
  allowed.push_back (
      biu::COption ("maxThreads", true, biu::COption::INT,
		    "Sets the maximum number of threads for parallelized computation.", "1"));
