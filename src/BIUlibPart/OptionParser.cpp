// $Id: OptionParser.cpp,v 1.1 2015/01/24 23:32:39 cvskinet Exp $
...
// Konstruktor
COptionParser::COptionParser(OptionMap    _options,
                             int          argc,
                             char         **argv,
                             std::string  infoText)
  :   opt(_options),
...
void
COptionParser::coutLineBreaking(std::string   text,
                                std::ostream &os,
                                ...
                                const int     lineLength) const
{
  ...
  while ((int)text.size() > (lineLength - emptyHeadSize)) {
    ...
    if (nextCutPos == text.npos) {
      // notwendige worttrennung
...
COptionParser::coutUsage() const
{
  ...
  for (i = 0; i < opt.size(); i++) {
    pInfo = std::string(opt[i].optional ? "  [-" : "   -") + opt[i].option + std::string(
      opt[i].retType == COption::BOOL ? "" : "=..");
...
bool
COptionParser::argExist(std::string option)
{
...
std::string
COptionParser::getStrVal(std::string arg)
{
  if (argExist(arg))
...
char
COptionParser::getCharVal(std::string arg)
{
  if (argExist(arg))
...
int
COptionParser::getIntVal(std::string arg)
{
  if (argExist(arg)) {
    int                 ret;
...
float
COptionParser::getFloatVal(std::string arg)
{
  if (argExist(arg)) {
    float               ret;
    std::istringstream  is;
...
double
COptionParser::getDoubleVal(std::string arg)
{
  if (argExist(arg)) {
    double              ret;
    std::istringstream  is;
...
bool
COptionParser::getBoolVal(std::string arg)
{
  return argExist(arg);
}
...
void
COptionParser::coutError(int          error,
                         ...
                         std::string  errormsg)
{
  ...
  switch (error) {
    ...
    case ERR_WR_USE:
      ...
      break;
    case ERR_WR_VAL:
      std::cout << "wrong argument type : ";
      break;
    case ERR_NO_ARG:
      std::cout << "needed argument not given : ";
      break;
    default:
...
bool
COptionParser::isCastable(std::string val,
                          int         type) const
{
...
// parst Parameter
void
COptionParser::parseOpt(int   argc,
                        char  **argv)
{
  ...
  for (i = 0; i < optStart.size(); i++) {
    ...
    if (j != opt.size()) {
      // dann option tatsaechlich verfuegbar
      if (opt[j].retType == COption::BOOL) {
        // extrabehandlund der booleans
        if (actOpt.size() > actOptName.size() + 1) {
          // dann falsche benutzung
        ...
        if (actOpt.find("=", 0) == std::string::npos) {
          // dann keine zuweisung obwohl kein bool
          ...
          } else if (isCastable(actVal, opt[j].retType)) {
            // wenn typecast moeglich
            opt[j].strValue = actVal;         // dann speichere
            opt[j].exist    = true;
          } else {
            // fehler da typecast nicht moeglich
