// $Id: MatrixSparse.hh,v 1.1 2015/01/24 23:32:39 cvskinet Exp $
...
namespace biu
{
  ...
  template<class T>
    ...
    public:
      ...
      //! @return *this
      MatrixSparseR&
      operator = (const MatrixSparseR<T> &mat);
      //! special case of *
      ...
      //!         false otherwise
      inline bool
      operator == (const MatrixSparseR<T> &mat) const;
      //! test for inequality
      //! @param mat the matrix to compare with
      ...
      //! @return the matrix element at position (r,c)
      inline T
      atConst (const size_t r, const size_t c) const;
      //! direct access to element (r,c)
      //! @param r the row to access
      //! @param c the column to access
      //! @return the matrix element at position (r,c)
      inline T&
      at (const size_t r, const size_t c);
      //! checks if element (r,c) is non-empty
      //! @param r the row to access
      //! @param c the column to access
      //! @return wether or not (r,c) exists
      inline bool
      exists (const size_t r, const size_t c) const;
      //! number of rows
      //! @return row number
      inline size_t
      numRows () const;
      ...
      void
      resize (const size_t row, const size_t col);
      ...
      void
      resize (const size_t row, const size_t col, const T& defVal);
      //! Sets the default value for new fields
      //! @param defVal the default value for new matrix elements
      void
      setDefaultValue (const T& defVal);
...
namespace biu
{
  ...
  template<class T>
    ...
    public:
      ...
      //! @return *this
      MatrixSparseC&
      operator = (const MatrixSparseC<T> &mat);
      //! special case of *
      //! column number has to equal vec size
      //! @param vec the column vector to multiply with
      //! @return a new column vector representing (this * vec)
      ...
      //!         false otherwise
      inline bool
      operator == (const MatrixSparseC<T> &mat) const;
      //! test for inequality
      //! @param mat the matrix to compare with
      //! @return true if dimensions or at least one element are is not equal,
      //!         false otherwise
