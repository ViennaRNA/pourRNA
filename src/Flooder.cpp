	...
	while (!pq.empty()) {
				...
				neighbor++)
		{
			...
			if ((neighborE < minNeighE)
					|| (neighborE == minNeighE
							&& StructureUtils::IsSmaller(neighborStructure,
									minNeigh))) {
			...
			if (neighborE <= MaxEnergy) // energy does not exceed maxE
				...
				if (neighborE > top->first.QueueState.energy
						|| (neighborE == top->first.QueueState.energy
								&& StructureUtils::IsGreater(neighborStructure,
										top->first.QueueState.structure))) { // greater in sequence order
		...
		if ((minNeighE == top->second.QueueState.energy
				&& StructureUtils::IsEqual(minNeigh,
						top->second.QueueState.structure)) // belongs to the basin
		|| top->second.QueueState.structure == NULL) // top is the local minimum
		{
			...
			for (NeighListPQ::const_iterator n = toStore.begin();
				...
				if (toFill == pq.end()) {
					...
					if (pq.size() >= MaxStatesToQueue) { // queue to big
			...
			if (&scSurface != NULL) {
				...
				for (NeighListPQ::const_iterator n = toCheck.begin();
						n != toCheck.end(); n++) {
